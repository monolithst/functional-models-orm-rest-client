...
const _createOrmModel = <T extends FunctionalModel>(
  model: Model<T>
): OrmModel<T> => {
  ...
  describe('#()', () => {
    ...
    describe('#delete()', () => {
      it('should use the httpProvider passed in', async () => {
        ...
        const modelInstance = TestModel.create({ name: 'My Name' })
        await instance.delete<TestModelType, Model<TestModelType>>(
          modelInstance
        )
        assert.isTrue(httpProvider.called)
      })
    ...
    describe('#bulkInsert()', () => {
      it('should use the httpProvider passed in', async () => {
        ...
        const modelInstance = TestModel.create({ name: 'My Name' })
        // @ts-ignore
        await instance.bulkInsert<TestModelType, Model<TestModelType>>(
          TestModel,
          [modelInstance]
        )
        assert.isTrue(httpProvider.called)
    ...
    describe('#search()', () => {
      it('should use the httpProvider passed in', async () => {
        ...
        // @ts-ignore
        await instance.search<TestModelType, Model<TestModelType>>(
          TestModel,
          {}
        )
        assert.isTrue(httpProvider.called)
